...
export function appendLatencySample({ serviceId, serviceType, observedAt = Date.now(), durationMs, success, statusCode = null }) {
  ...
  const normalizedStatusCode = safeStatusCode(statusCode);
  ...
  const result = write();
  return { id: result.lastInsertRowid, serviceId: id, serviceType: type, observedAt: timestamp, durationMs: Math.round(duration), success: Boolean(success), statusCode: normalizedStatusCode };
}
